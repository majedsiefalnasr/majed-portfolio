...
import {
  getTemplateById,
  validateTemplate,
} from '~/app/utils/content-generator'
...
if (
  normalizedChoice === 'a' ||
  normalizedChoice === '' ||
  normalizedChoice.includes('default')
) {
  selectedTemplate = getTemplateById(
    contentType === 'blog' ? 'blog-default' : 'case-study-default',
  )
...
if (refinementCount >= MAX_REFINEMENTS) {
  // Show warning
  console.log(
    `⚠️  You've reached the maximum of ${MAX_REFINEMENTS} refinements.\n`,
  )
  console.log(
    '**Recommended:** Option A - Save the current version and refine manually if needed.\n',
  )
  console.log('| Option | Description |')
  console.log('|--------|-------------|')
